 ...
 */
std::tuple<int, int, int> find_longest_consecutive_ones(uint64_t high,
                                                        uint64_t low) {
    int max_len = 0;
    int max_start = -1;
    int max_end = -1;
...
int main() {
    ...
    // The number now has 64 ones (high) and 6 ones (low) which is not a single
    // contiguous run. Let's create a single long run.
    high_val = ((uint64_t)-1) >>
               (64 - 70 + 64);  // This is complex, let's use a simpler example
    high_val = 0;
    low_val = 0;
    ...
    std::cout << "Finding longest run of set bits in a 128-bit number (using "
                 "two 64-bit integers)."
              << std::endl;
    auto [len, start, end] =
        find_longest_consecutive_ones(high_example, low_example);

    std::cout << "The longest run has a length of " << len << "." << std::endl;
    std::cout << "It starts at index " << start << " and ends at index " << end
              << "." << std::endl;
