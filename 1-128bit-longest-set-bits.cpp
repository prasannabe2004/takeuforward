...
int main() {
    ...
    /*
        ...
        u128 val2 = (u128(1) << 60) | (u128(1) << 61) | (u128(1) << 62) |
       (u128(1) << 63) | (u128(0b1111) << 10); auto [len2, start2, end2] =
       longest_consecutive_ones_with_indices(val2); std::cout << "Number: " <<
       val2 << std::endl; std::cout << "Longest run: Length " << len2 << ", from
       index " << start2 << " to " << end2 << std::endl << std::endl;
        // The run of 1s from index 60 to 63 has length 4, which is longer than
       the run at index 10-13.
        ...
        std::cout << "Longest run: Length " << len3 << ", from index " << start3
       << " to " << end3 << std::endl << std::endl;
        // This is the number with all lower 64 bits set. Length is 64, indices
       0-63.
    */
    return 0;
}
